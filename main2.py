...
lumpy_model = models.mobilenet_v2(pretrained=False)
lumpy_model.classifier[1] = nn.Sequential(
    nn.Linear(lumpy_model.last_channel, 64),
    nn.ReLU(),
    nn.Dropout(0.7),
    nn.Linear(64, 1),
    nn.Sigmoid()
)
lumpy_model.load_state_dict(torch.load("final_model.pth", map_location=device))
lumpy_model = lumpy_model.to(device)
lumpy_model.eval()
    ...
    for r in results:
        for box in r.boxes:
            ...
            if label == "cow":
                ...
                img_tensor = inference_transform(crop).unsqueeze(0).to(device)
                with torch.no_grad():
                    prediction = lumpy_model(img_tensor).item()
...
def predict_latest():
    try:
        ...
        features = [
            latest_doc["Temp"], latest_doc["MQ"],
            latest_doc["Accel_X"], latest_doc["Accel_Y"], latest_doc["Accel_Z"],
            latest_doc["Gyro_X"], latest_doc["Gyro_Y"], latest_doc["Gyro_Z"],
            latest_doc["pH"], latest_doc["Pulse"],
            latest_doc["Accel_Mag"], latest_doc["Gyro_Mag"]
        ]
